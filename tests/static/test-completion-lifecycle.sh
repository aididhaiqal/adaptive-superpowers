...
require_text "$EXECUTE" 'canonical current record'
require_text "$FINISH" 'required completion review has no unresolved blocking findings'
require_text "$FINISH" 'applicable project state is reconciled'
require_text "$FINISH" 'A successful authorized local merge retires the exact clean task-owned worktree by default.'
require_text "$FINISH" 'explicitly asks to retain it'
require_text "$FINISH" 'Verify the merged result before worktree or branch removal.'
require_text "$FINISH" 'preserve both the worktree and feature branch'
require_text "$FINISH" 'clean, including untracked files'
require_text "$FINISH" 'git worktree remove'
require_text "$FINISH" 'git worktree remove <exact-task-worktree-path> &&'
require_text "$FINISH" 'git worktree prune'
require_text "$FINISH" 'If worktree removal fails, stop retirement; preserve the worktree and branch.'
require_text "$FINISH" 'git -C <exact-task-worktree-path> status --porcelain -uall'
require_text "$FINISH" 'Never automatically retry with `--force`.'
require_text "$FINISH" 'Resolve only within existing authority; otherwise report the blocker and ask.'
require_text "$FINISH" 'git branch -d <feature-branch>'
require_text "$FINISH" 'Never force-delete through this path.'
...
require_text "$STATE" 'At the start of material or resumable work'
require_text "$STATE" 'If none exists, use or create one `docs/progress.md`'
require_text "$STATE" 'implemented, tested, committed, pushed, merged, deployed, and runtime-verified'
require_text "$STATE_MODEL" 'Plans and task reports remain intent or supporting evidence'
