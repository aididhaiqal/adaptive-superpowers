...
Implementation completion follows one shared lifecycle: technical verification, one proportional final review for omissions and material recommendations, then evidence-backed completion. Fast and mechanical work can use a bounded implementer review; material work uses a fresh reviewer when available. Material plan execution, concurrent implementation, and editing delegation require task-scoped isolation. New material tasks receive dedicated worktrees; reuse requires an attributable continuation on the expected unmerged branch. After resume, revalidate live state and classify the remaining work; routine work does not require escalation solely because context compacted. Revalidate identity before editing batches or branch operations and on external-change signals. Branch delivery follows only after this gate and only with user or repository authority. A verified local merge retires only the exact clean owned worktree and fully merged feature branch by default; PR, explicit retention, dirty, inherited, detached, wrong-base, and ambiguous states remain preserved.
...
The bundle does not ship mandatory parallel-agent or subagent-driven workflows. Conditional delegation guidance covers independent ownership, same-shape batching, nested scope inheritance, duplicate-review prevention, and reuse of implementers for fixes. It does not impose an agent count or depth cap. The host owns tools, roles, model/effort selection, and orchestration; coupled work stays with its owner, while the parent retains integration and user decisions. The bundle also omits the browser visual companion until its project-state files are written without following untrusted symlinks and that behavior is covered by tests.

Retained coverage need not mean a new test edit. Existing sufficient tests count; independent expectations and realistic fault detection matter more than test quantity. Structural lint checks packaging and intentional wording, while fresh consuming-agent probes establish behavior. A successful fixture or an older benchmark does not prove the current cross-model workflow.
