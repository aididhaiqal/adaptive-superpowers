...
Repository fixtures cover retained feature tests, confirmed-regression debugging, stale-resume checkout safety, persistent-goal planning/verification cadence, routine project-state no-churn, and protected future-watch retention. The persistent fixture asserts one workstream plan, retained focused evidence, bounded broader-suite use, and no adjacent import/export implementation. The project-state pair asserts that a routine fix creates no bookkeeping while completion of neighboring tracked work preserves an unrelated accepted obligation. Their live cross-model Quorum runs remain pending.
