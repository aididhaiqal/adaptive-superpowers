...
The Windows-hook intent and core test-quality guidance from the older staged draft are selectively incorporated in `3944b17`. Outcome 5 of the governing plan now covers removal-refusal handling and dependency-boundary test guidance. The exhaustive ruling log, universal test-reference loading, reference rename, and blanket source-text prohibition are not adopted. Preserve the old worktree unchanged as reconciliation evidence; native Windows checks and the draft's other deferred decisions remain in the canonical record. Neither this study nor the local follow-up authorizes deleting it.
