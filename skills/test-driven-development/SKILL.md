...
Coverage is broader than this workflow's trigger: observable behavior changes retain sufficient automated coverage, which existing tests may supply unchanged. This skill governs red-green order only when its trigger matches.
...
Test public outputs, state, errors, or durable effects. Read [testing-anti-patterns.md](testing-anti-patterns.md) for nontrivial test design, questionable coverage, mocks, doubles, or test-only production APIs.
