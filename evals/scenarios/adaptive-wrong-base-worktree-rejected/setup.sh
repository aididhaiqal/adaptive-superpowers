...
printf '%s\n' 'expected main base' > "$main_repo/src/main-base.js"
git -C "$main_repo" add src/main-base.js
git -C "$main_repo" commit -qm 'chore: advance intended base'
expected_fork="$(git -C "$main_repo" rev-parse HEAD)"

git -C "$main_repo" switch -qc release-base HEAD~1
mkdir -p "$main_repo/src"
printf '%s\n' 'release-only work' > "$main_repo/src/release-only.js"
git -C "$main_repo" add src/release-only.js
git -C "$main_repo" commit -qm 'chore: advance release base'
